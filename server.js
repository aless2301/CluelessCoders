...
// --- 3. "MIDDLEWARE" ---
// Esto permite que el servidor procese datos en formato JSON (como el nombre del usuario)
...
app.post('/revisar-circuito-jpg', upload.single('circuitoJpg'), async (req, res) => {
    try {
        ...
        // --- LÓGICA NUEVA: DETERMINAR SI FUE APROBADO ---
        // Comprobamos si la respuesta de la IA contiene la palabra clave "APROBADO"
        const isApproved = respuestaDeIA.includes('APROBADO');

        // Enviamos la respuesta de la IA de vuelta al navegador, incluyendo el estado de aprobación
        res.json({
            feedback: respuestaDeIA,
            approved: isApproved
        });
...
// --- NUEVA RUTA: RECIBIR Y PROCESAR EL NOMBRE DEL USUARIO ---
// Esta ruta espera un JSON con el campo 'nombre'
app.post('/guardar-nombre', (req, res) => {
    try {
        ...
        if (!nombreUsuario) {
            return res.status(400).json({ status: 'error', message: 'No se proporcionó un nombre de usuario.' });
        }

        console.log(`¡Logro registrado! El usuario "${nombreUsuario}" aprobó su circuito.`);

        // Enviamos una respuesta de éxito al cliente
        res.json({ status: 'success', message: 'Nombre guardado correctamente.' });
    } catch (error) {
